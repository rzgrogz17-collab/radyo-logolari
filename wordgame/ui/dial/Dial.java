...
public class Dial extends Group implements Disposable {
        ...
        if (!dialButton.isLineSnapped){
            ...
            if (selectedButtons.size >= 2){
                if (dialButton.id == selectedButtons.get(selectedButtons.size - 2).id){
                    // Play the note for the connection being undone (the same
                    // note that played when it was made) before removing it,
                    // so backing out plays the sequence in reverse.
                    if(!ConfigProcessor.muted) sfx();
    ...
    public InputListener inputListener = new InputListener(){
        ...
        public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
            ...
            if(gameScreen != null) gameScreen.onDialPointerDown();
        ...
        public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
            dialButtonMouseUp();
            if(gameScreen != null) gameScreen.onDialPointerUp();
        }
    };
    ...
    public void shuffle(){
        ...
        // Derangement: her harf kendi açısına dönmesin. Sonsuz döngüye
        // düşmemek için deneme sınırı var; başarısızsa yine de karıştırılmış
        // diziyi kullanır.
        boolean deranged = false;
        for(int attempt = 0; attempt < 32 && !deranged; attempt++){
            angles.shuffle();
            deranged = true;
            for(int i = 0; i < dialButtons.size; i++){
                if(dialButtons.get(i).angle == angles.get(i)){
                    deranged = false;
                    break;
                }
            }
